        ...
        <AnimatePresence initial={false}>
            <motion.nav className='flex items-center justify-center gap-4'>
                <motion.div className='fixed z-50 top-20 right-[50%] flex flex-col items-center justify-center text-neutral-50 max-w-5xl'>
                    <motion.menu
                        className={isOpen ? `fixed top-[119px] text-black max-w-5xl w-full z-50 min-h-screen h-auto grid grid-cols-7 xs:grid-cols-8 sm:grid-cols-7 md:grid-cols-10 lg:grid-cols-9 xl:grid-cols-10 xs:grid-rows-16 sm:grid-rows-12 md:grid-rows-10 lg:grid-rows-10 xl:grid-rows-11 duration-1000 ease-in-out bg-black` :
                            `fixed top-[-100%] text-black max-w-5xl w-full z-50 min-h-screen h-auto grid grid-cols-7 xs:grid-cols-8 sm:grid-cols-7 md:grid-cols-10 lg:grid-cols-9 xl:grid-cols-10 xs:grid-rows-16 sm:grid-rows-12 md:grid-rows-10 lg:grid-rows-10 xl:grid-rows-11 duration-1000 ease-in-out bg-black`}
