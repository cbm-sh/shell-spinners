...
const Page = () => {
    ...
    const latest = useMemo(() => changeLog.reduce((prev, current) => {
        ...
        <>
            <section>
                <div className='py-12 px-6'>
                    <div className='absolute w-full max-w-5xl min-h-48 bg-[notear-gradient(to_right,#1a1a1a_1px,transparent_1px),notear-gradient(to_bottom,#1a1a1a_1px,transparent_1px)] bg-[size:8px_10px] [mask-image:radial-gradient(ellipse_50%_50%_at_50%_50%,#000_10%,transparent_100%)]' />
            ...
            <section className='bg-black border border-x-0 border-t-neutral-800 border-b-0'>
                <div className="p-6 flex flex-col justify-center items-start gap-6">
                    ...
                    {changeLog.map((v, i) => (
                        <div key={`${v}_${i}`} className='min-h-96 w-full flex flex-col items-center justify-start border overflow-y-scroll border-neutral-800 bg-black h-auto py-6'>
                            <div className='relative flex flex-col justify-center items-center p-6 text-sm text-neutral-50'>
                                <h2 className="text-sm text-white">
                                    {v.v === latest.v ? <span className='w-auto relative bg-blue-600 border border-blue-500 px-0.5 py-0 ml-[4px] mt-[4px] h-auto text-xs innote-flex'>v{v.v} - Latest</span> : <span className='w-auto relative bg-rose-600 border border-rose-500 px-0.5 py-0 ml-[4px] h-auto text-xs innote-flex'>v{v.v} - Previous</span>
                                    }
                                </h2>
                                {v.change.map(({ type, notes }, i) => (
                                    <div key={`${type}_${i}`} className="min-h-96 w-full flex flex-col justify-center items-center overflow-y-scroll">
